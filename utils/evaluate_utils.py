...
@torch.no_grad()
def hungarian_evaluate_ours(predictions, targets, offset=0):
    # Hungarian matching
    targets = targets - offset
    predictions = predictions - offset
    predictions_np = predictions.numpy()
    ...
    reordered_preds = reordered_preds.numpy()
    acc = int((reordered_preds == targets_sel.numpy()).sum()) / float(num_elems) #accuracy is normalized with the total number of samples not only the valid ones
    fpl = false_pl_rate(reordered_preds,targets_sel.numpy(),num_classes)
    
    nmi = metrics.normalized_mutual_info_score(targets.numpy(), predictions.numpy()) # This can be useful to measure the agreement of two independent label assignments strategies on the same dataset when the real ground truth is not known.
    ari = metrics.adjusted_rand_score(targets.numpy(), predictions.numpy()) # The adjusted Rand index is thus ensured to have a value close to 0.0 for random labeling independently of the number of clusters and samples and exactly 1.0 when the clusterings are identical (up to a permutation).
    
    return {'acc': acc*100,'fpl':fpl, 'ari': ari, 'nmi': nmi, 'hungarian_match': match}    

@torch.no_grad()
def false_pl_rate(preds,targets,num_classes):
    fpl = []
    for i in range(num_classes):
        fpl_n = ((preds==targets) * (targets == i)).sum()
        t_n = (targets == i).sum()
        fpl.append(fpl_n / t_n)
    return fpl
